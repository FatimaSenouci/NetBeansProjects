...
import static Synchronized.Synchronisation.Synchro.Main.*;
...
public class Avion extends Thread {
    ...
    Avion (Semaphore machine, String name, Semaphore mutex) {
    ...
    public void waitingMachine() {
        try {
            ...
            processesTextArea.setText(processesTextArea.getText() + this.getName() + " is Waiting To Fly ...\n");
    ...
    public void usingMachine() {
        try {
            ...
            for(int i = 0; i < manName.length; i++)
                if (this.getName().equals(manName[i])) {
                    translateTransition(350 + (4 - i) * 90, 10, manImage[i]);
                }
            
            mutex.acquire();
            processesTextArea.setText(processesTextArea.getText() + this.getName() + "  Using The Piste ...\n");
            mutex.release();
            sleep(3000);
        } catch (InterruptedException ex) {}
    ...
    public void releaseMachine() {
        try {
            ...
            for(int i = 0; i < manName.length; i++)
                if (this.getName().equals(manName[i])) {
                    translateTransition(400,320, manImage[i]);
                    break;
                }
            
            processesTextArea.setText(processesTextArea.getText() + this.getName() + " Release The Pist \n");
           mutex.release();
